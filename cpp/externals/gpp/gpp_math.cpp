...
\endrst*/
void BuildCovarianceMatrix(const CovarianceInterface& covariance,
                                               double const * restrict points_sampled,
...
void GaussianProcess::RecomputeCholeskyVariables() {
  // resize if needed
  if (unlikely(static_cast<int>(K_inv_y_.size()) != num_sampled_*(num_derivatives_+1))) {
    K_chol_.resize(Square(num_sampled_*(num_derivatives_+1)));
    K_inv_y_.resize(num_sampled_*(num_derivatives_+1));
...
void GaussianProcess::RecomputeMeanVariables(bool mean_change /* = true*/) {
  // resize if needed
  if (unlikely(static_cast<int>(K_inv_y_.size()) != num_sampled_*(num_derivatives_+1))) {
    K_inv_y_.resize(num_sampled_*(num_derivatives_+1));
  }

  // recompute mean quantities
  if (mean_change == true) {
    mean_ = 0.0;
    for (int i=0; i<num_sampled_; ++i){
       mean_ += points_sampled_value_[i*(num_derivatives_+1)];
...
void GaussianProcess::FillPointsToSampleState(StateType * points_to_sample_state) const {
  ...
  if (points_to_sample_state->num_derivatives > 0) 
  {
    ...
    if (points_to_sample_state->precomputed_grad_K_inv_times_K_star){
      ...
      double * restrict gKs_temp_2 = points_to_sample_state->grad_K_star.data();
      ...
      for (int index = 0; index < col; index++){
        ...
        MatrixTranspose(gKs_temp_2 + index*row*dim_, dim_, row, transpose_temp.data());
...
void GaussianProcess::AddSampledPointsToGP(double const * restrict new_points,
                                           double const * restrict new_points_value,
                                           int num_new_points) {
  // update sizes
  num_sampled_ += num_new_points;
  ...
  // TODO(GH-192): Insert the new covariance (and cholesky covariance) rows into the current matrix  (O(N^2))
  // instead of recomputing everything (O(N^3)).
  RecomputeCholeskyVariables();
}

void GaussianProcess::NewSampledValue(double const * restrict new_points_value,
                                      int num_new_points,
                                      int sampling_point_index,
                                      bool mean_change /* = true*/) {
  std::copy(new_points_value, new_points_value + num_new_points*(num_derivatives_+1),
            points_sampled_value_.data() + sampling_point_index*(num_derivatives_+1));

  RecomputeMeanVariables(mean_change);
}
...
void PointsToSampleState::SetupState(const GaussianProcess& gaussian_process_in, double const * restrict points_to_sample_in,
  ...
  if (unlikely(num_sampled != gaussian_process_in.num_sampled())) {
    num_sampled = gaussian_process_in.num_sampled();
    K_star.resize((num_to_sample*(num_gradients_to_sample+1))*(num_sampled*(num_gradients_sampled+1)));
    grad_K_star.resize(num_derivatives*(num_sampled*(num_gradients_sampled+1)*(num_gradients_to_sample+1))*dim);
    grad_K_inv_times_K_star.resize(num_derivatives*(num_sampled*(num_gradients_sampled+1)*(num_gradients_to_sample+1))*dim);
    V.resize((num_to_sample*(num_gradients_to_sample+1))*(num_sampled*(num_gradients_sampled+1)));
  ...
  gaussian_process_in.FillPointsToSampleState(this);
}

PointsToSampleState::PointsToSampleState(const GaussianProcess& gaussian_process_in,
    ...
    : dim(gaussian_process_in.dim()),
      ...
      num_gradients_to_sample(num_gradients_to_sample_in),
      num_gradients_sampled(gaussian_process_in.num_derivatives()),
  ...
  SetupState(gaussian_process_in, points_to_sample_in, num_to_sample_in, num_gradients_to_sample_in, num_derivatives_in);
}
