...
class ExceptionEvaluator final : public SimpleObjectiveFunctionEvaluator {
 public:
  ...
  double GetOptimumValue() const noexcept override OL_PURE_FUNCTION OL_WARN_UNUSED_RESULT {
    return 0.0;
  }

  void GetOptimumPoint(double * restrict point) const noexcept override OL_NONNULL_POINTERS {
    point[0] = 0.0;
  }
  ...
  void ComputeHessianObjectiveFunction(StateType * OL_UNUSED(quadratic_dummy_state), double * restrict hessian_objective) const override OL_NONNULL_POINTERS {
...
int RunSimpleObjectiveOptimizationTests(OptimizerTypes optimizer_type) {
  switch (optimizer_type) {
    ...
    default: {
      OL_ERROR_PRINTF("%s: INVALID optimizer_type choice: %d\n", OL_CURRENT_FUNCTION_NAME, static_cast<int>(optimizer_type));
      return 1;
    }
  }
}
