 ...
 public:
  ...
  /*!\rst
    ...
    \param
      ...
      :points_sampled_value[num_sampled*(num_derivatives+1)]: values and derivatives of the already-sampled points,
        if only values are available, they should be used by setting points_sampled_value[i*(num_derivatives+1)] = value[i],
        when derivatives componens of points_sampled_value could be set to 0.
      :noise_variance[num_derivatives + 1]: the ``\sigma_n^2`` (noise variance) associated w/observation, points_sampled_value
...
//  explicit GaussianProcess(const GaussianProcess& source);
 ...
 private:
  ...
  void BuildMixCovarianceMatrix(double const * restrict points_to_sample, int num_to_sample, int const * restrict derivatives_to_sample,
                                int num_derivatives_to_sample, double * restrict cov_mat) const noexcept;
...
OL_NONNULL_POINTERS void ComputeOptimalPointsToSampleViaMultistartGradientDescent(
  ...
  if (num_to_sample == 1 && num_being_sampled == 0) {
    ...
    for (int i = 0; i < static_cast<int>(EI_starting.size()); ++i) {
      if (i < k){
    ...
    *found_flag = io_container.found_flag;
    ...
    // maximum possible number of indices
    k = std::min( k, static_cast<int>(q.size()) );
