...
using ::utils::Json;
using ::solver::PTParameterFree;
using ::solver::ParameterFreeLinearSearchSolver;

TEST(PTParameterFree, PuboModel)
{
  ...
  ParameterFreeLinearSearchSolver<model::PuboWithCounter<uint8_t>,
                      PTParameterFree<model::PuboWithCounter<uint8_t>>>
      pa_pf;
...
TEST(PTParameterFree, PuboEmptyModel)
{
...
TEST(PTParameterFree, IsingModel)
{
...
TEST(PTParameterFree, PuboEmptyMixedModel)
{
...
TEST(PTParameterFree, IsingEmptyMixedModel)
{
...
TEST(PTParameterFree, PuboGroupedModel)
{
  ...
  ParameterFreeLinearSearchSolver<model::PuboGrouped<uint64_t>,
                      PTParameterFree<model::PuboGrouped<uint64_t>>>
      pa_pf;
...
TEST(PTParameterFree, IsingGroupedModel)
{
  std::string input_file(utils::data_path("isinggrouped1.json"));
  std::string param_file(utils::data_path("pf5s.json"));
  ParameterFreeLinearSearchSolver<model::IsingGrouped, PTParameterFree<model::IsingGrouped>>
      pa_pf;
  model::IsingGrouped model;
  auto result =
      run_solver<ParameterFreeLinearSearchSolver<model::IsingGrouped,
                                     PTParameterFree<model::IsingGrouped>>>(
          pa_pf, model, input_file, param_file);
...
#ifndef _DEBUG
TEST(PTParameterFree, HonoringTimeout)
{
