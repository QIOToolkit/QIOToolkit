...
using ::utils::Json;
using ::solver::SAParameterFree;
using ::solver::ParameterFreeLinearSearchSolver;

TEST(SAParameterFree, PuboModel)
{
  ...
  ParameterFreeLinearSearchSolver<model::PuboWithCounter<uint8_t>,
                      SAParameterFree<model::PuboWithCounter<uint8_t>>>
      pa_pf;
...
TEST(SAParameterFree, PuboEmptyModel)
{
...
TEST(SAParameterFree, IsingModel)
{
...
TEST(SAParameterFree, PuboEmptyMixedModel)
{
...
TEST(SAParameterFree, IsingEmptyMixedModel)
{
...
TEST(SAParameterFree, PuboGroupedModel)
{
  ...
  ParameterFreeLinearSearchSolver<model::PuboGrouped<uint64_t>,
                      SAParameterFree<model::PuboGrouped<uint64_t>>>
      pa_pf;
...
TEST(SAParameterFree, IsingGroupedModel)
{
  std::string input_file(utils::data_path("isinggrouped1.json"));
  std::string param_file(utils::data_path("pf5s.json"));
  ParameterFreeLinearSearchSolver<model::IsingGrouped, SAParameterFree<model::IsingGrouped>>
      pa_pf;
  model::IsingGrouped model;
  auto result =
      run_solver<ParameterFreeLinearSearchSolver<model::IsingGrouped,
                                     SAParameterFree<model::IsingGrouped>>>(
          pa_pf, model, input_file, param_file);
...
#ifndef _DEBUG
TEST(SAParameterFree, HonoringTimeout)
{
